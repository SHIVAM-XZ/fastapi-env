...
def show(db:Session=Depends(get_db),current_user:schemas.user=Depends(get_current_user)):
...
def discard(id:int, db:Session=Depends(get_db),current_user:schemas.user=Depends(get_current_user)):
    return blog.discard(id,db)
